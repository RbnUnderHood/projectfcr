...
var CURRENCY_SYMBOLS = {
  PYG:'₲', USD:'$', BRL:'R$', ARS:'AR$', UYU:'$U', PEN:'S/', BOB:'Bs',
  COP:'$', CLP:'$', MXN:'$', PAB:'B/.', EUR:'€', GBP:'£'
};
...
try { calculationHistory = JSON.parse(localStorage.getItem(STORAGE.history) || '[]'); }
catch(e){ calculationHistory = []; }

var calendarEvents = [];
try { calendarEvents = JSON.parse(localStorage.getItem(STORAGE.calendar) || '[]'); }
catch(e){ calendarEvents = []; }

var defaultCurrency = localStorage.getItem(STORAGE.currency) || 'PYG';

var flocks = [];
try { flocks = JSON.parse(localStorage.getItem(STORAGE.flocks) || '[]'); }
catch(e){ flocks = []; }

/* ========= Persistence helpers ========= */
function saveHistory(){ localStorage.setItem(STORAGE.history, JSON.stringify(calculationHistory)); }
function saveCalendar(){ localStorage.setItem(STORAGE.calendar, JSON.stringify(calendarEvents)); }
function saveFlocks(){ localStorage.setItem(STORAGE.flocks, JSON.stringify(flocks)); }
function setCurrency(code){ defaultCurrency = code; localStorage.setItem(STORAGE.currency, code); }
