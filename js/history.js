...
function renderHistoryTable() {
  ...
  for (var i = 0; i < sorted.length; i++) {
    ...
    var titleFeed =
      _fmtKgShort(row.feedAmount) +
      (row.altFeedKg && row.altFeedKg > 0 ? '/' + _fmtKgShort(row.altFeedKg) : '');

    html +=
      '' +
      '<tr class="row-click" onclick="openEdit(' +
      idx +
      ')" title="Tap to edit">' +
      '<td>' +
      row.date +
...
function clearHistory() {
  ...
  window.calendarEvents = window.calendarEvents.filter(function (e) {
    return e.type !== 'fcr';
  });
  if (typeof saveCalendar === 'function') saveCalendar();
  renderHistoryTable();
  if (hasClassId('calendarTab', 'active') && typeof window.renderCalendar === 'function')
...
function showDeleteConfirm(i) {
  _historyDeleteIndex = i;
  showModal('deleteModal');
}
function cancelDelete() {
  _historyDeleteIndex = null;
...
function confirmDelete() {
  ...
  if (Array.isArray(window.calendarEvents)) {
    window.calendarEvents = window.calendarEvents.filter(function (e) {
      return !(e.type === 'fcr' && e.refId === old.id);
    });
    if (typeof saveCalendar === 'function') saveCalendar();
  }
  renderHistoryTable();
...
function openEditNoteForDate(dateStr) {
  ...
  var evs = {};
  try {
    evs = JSON.parse(localStorage.getItem('fcrCalendarEvents') || '{}');
  } catch (e) {}
  var day = evs[dateStr];
  ...
  var n = byId('editNotes');
  if (n) n.value = noteTxt;
...
function saveEdit() {
  ...
  if (window.editNoteOnly) {
    ...
    if (bx) bx.style.display = 'none';
    var ttl = document.querySelector('#editModal .modal-title');
    if (ttl) ttl.textContent = '✏️ Edit Entry';
  ...
  try {
    ...
    if (!isFinite(feedVal) || !isFinite(eggsVal)) {
      alert('Invalid numbers.');
      return;
    }
    if (eggWVal < 10 || eggWVal > 200) {
      alert('Please set a realistic egg weight (10–200 g).');
      return;
    }
    // Alt feed (Edit Entry)
    ...
    var altFeedName = altOn
      ? String((byId('editAltName') && byId('editAltName').value) || '').trim()
      : '';
    ...
    try {
      if (typeof window.addOrUpdateFcrEvent === 'function')
        window.addOrUpdateFcrEvent(finalRec, old.id);
    } catch (calErr) {
    ...
    if (typeof window.openDay === 'function') {
      ...
      var iso =
        window.selectedDate instanceof Date ? _toLocalISO(window.selectedDate) : finalRec.date;
      window.showEventsForDate(iso);
    }

    // Repaint calculator to match this edited record
    if (typeof window.paintCalculatorFromRecord === 'function')
      window.paintCalculatorFromRecord(finalRec);
